...
public class LCS_LongestCommonSubsequence {
	...
	/* Print the LCS for X[0..m-1], Y[0..n-1] */
	static void printShortestCommonSuperSequence(char[] X, char[] Y, int m, int n) {
		int L[][] = iterativeLCS(X, Y, m, n);

		// Following code is used to print SCS
		int lengthOfSCS = m + n - L[m][n];

		// Create a character array to store the lcs string
		char[] scs = new char[lengthOfSCS];
		// lcs[index] = ''; // Set the terminating character
		...
		// Print the lcs
		System.out.print("SCS of " + String.valueOf(X) + " and " + String.valueOf(Y) + " is ");
		System.out.print(String.valueOf(scs));
	}
